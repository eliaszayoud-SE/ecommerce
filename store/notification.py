...
def _get_access_token():
  ...
  "project_id": f"{os.environ.get('PROJECT_ID')}",
  "private_key_id": f"{os.environ.get('PRIVATE_KEY_ID')}",
  "private_key": f"{os.environ.get('PRIVATE_KEY')}",
  "client_email": f"{os.environ.get('CLIENT_EMAIL')}",
...
def send_notification(title, message, topic, pageid, pagename):
  ...
  conn.request("POST", f"/v1/projects/ecommerce-31838/messages:send", payload, headersList)
  print('7')
